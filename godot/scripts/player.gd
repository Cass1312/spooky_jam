...
@export var MOVE_SPEED: float = 64; # Tiles per second
...
const DODGE_LENGTH: float = 0.4;
const DODGE_COOLDOWN: float = 1;
var DODGE_SPEED = 6;
...
func _process(delta: float):
	...
	if dodge_timer < (DODGE_COOLDOWN - DODGE_LENGTH) && self.velocity.length_squared() > 0:
...
func _physics_process(delta: float):
	...
	if dodge_buffered || Input.is_action_just_pressed("Player - DODGE"):
		if dodge_timer <= 0:
			...
			var spin = create_tween().bind_node(self).set_trans(Tween.TRANS_LINEAR);
			spin.tween_property(self, "rotation", TAU, DODGE_LENGTH);
